...
JFreeChart chart	=	ChartFactory.createPieChart3D("OrderMethod", dataset, true, true, false);
  ...
  //convert to image	
	 try {
		ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/ompie.jpg"), chart, 200, 250);
	} catch (IOException e) {
	...
	///2.CREATING BAR CHART
		...
		JFreeChart chart =	ChartFactory.createBarChart("OrderMethod", " MODE type", "COUNT", dataset);
		
		    //3.save image
		  try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/ombar.jpg"), chart, 300, 400);
		} catch (IOException e) {
